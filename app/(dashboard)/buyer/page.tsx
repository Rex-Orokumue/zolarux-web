...
export default function BuyerDashboardPage() {
  ...
  const fetchDashboardData = async (userId: string, email: string, silent = false) => {
    ...
    try {
      ...
      const { data: cartData } = await supabase
        ...
        .eq('buyer_id', userId)
      if (cartData) {
        setCartCount(cartData.reduce((sum, item) => sum + (item.quantity || 0), 0))
      }
  ...
  const handleRefresh = () => {
    if (user) fetchDashboardData(user.id, user.email || '', true)
  }
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      ...
      {/* Referral Widget */}
      {user && (
        <ReferralWidget userId={user.id} userType="buyer" />
      )}
